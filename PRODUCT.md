...
- Contact: a `/contact` page with the address only (no form, no per-project routing). contact@multnelis.org exists as a Cloudflare Email Routing address forwarding to the organisation owner (bacuongtr@gmail.com); there is no hosted mailbox and no sending from that address.
